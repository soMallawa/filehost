# -*- coding: utf-8 -*-
"""
CPT-only for Gemma-3 4B IT — Optimized for H100 SXM with GGUF export
---------------------------------------------------------------------
- Base: unsloth/gemma-3-4b-it (instruction-tuned)
- Task: Continued pretraining (domain adaptation) on Sinhala Wikipedia
- Precision: bf16 on H100; 4-bit loading + LoRA
- Batch: per-device 16, grad-accum 1 (global ≈ 16)
- TRAIN_FRACTION controls corpus size (default 0.20 = 20%)
...
# Batch / steps
PER_DEVICE_BS = int(os.getenv("PER_DEVICE_BS", "16"))
...
TRAIN_FRACTION = float(os.getenv("TRAIN_FRACTION", "0.25"))  # 20% by default
...
OUTPUT_DIR = os.getenv("OUTPUT_DIR", "outputs_h100_cpt_only")
SAVE_DIR = os.getenv("SAVE_DIR", "gemma3_4b_it_cpt_h100")
...
print("==== Starting CPT (H100, CPT-only) ====")
stats = trainer.train()
