...
CHECKPOINT_FILE = "resume_checkpoint.json"  # File to store processed message IDs
PINECONE_API_KEY = os.getenv("PINECONE_API_KEY", "pcsk_3eBUzc_GmdvFhHndN4zAXejwE715zbC99jhjLyxjgn9Dxwdxc5Fwq4yBPQCBKKXsUqKzzP")
PINECONE_INDEX_NAME = os.getenv("PINECONE_INDEX_NAME", "febchatv2")
PINECONE_REGION = "us-east-1"  # Change if needed
BATCH_SIZE = 1000  # Pinecone batch limit
...
def process_file(file_path):
    """Extracts messages from a file, generates embeddings, and uploads them to Pinecone in batches."""
    try:
        ...
        for message in messages:
            ...
            message_id = f"{sender}_{timestamp}"  # Unique ID

            # Skip already processed messages
            if message_id in uploaded_message_ids:
                logging.debug(f"Skipping already processed message: {message_id}")
                continue

            # Ignore shared content (reels, links, attachments)
            if "share" in message or not content:
                logging.debug(f"Ignoring shared content from {sender} at {timestamp}")
                continue
        ...
        if vectors:
            ...
            save_checkpoint()  # Save progress after successful upload
...
def upload_batches(vectors):
    ...
    with ThreadPoolExecutor() as executor:
        for i in range(0, len(vectors), BATCH_SIZE):
            ...
            executor.submit(index.upsert, batch)
...
def process_all_files():
    ...
    for file in files:
        logging.info(f"Starting file: {file}")
